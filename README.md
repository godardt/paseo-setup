...
When Paseo is detected, the installer also merges a **Claude Peppy** provider (`agents.providers.claude-peppy`) into `$PASEO_HOME/config.json`. It extends Paseo's regular Claude provider — the native model list, auto mode, WebSearch, and normal usage reporting all work — and differs only in the environment it launches with: `CLAUDE_CONFIG_DIR` pointing at the peppy profile, plus a `CLAUDE_CODEX_MODEL_LABEL_PREFIX` marker. Select **Claude Peppy** when creating an agent to use the second account; the regular **Claude** provider keeps using the daemon's own profile. Its model picker shows the native models with a **Peppy** prefix — **Peppy Opus 5**, **Peppy Fable 5.1**, and so on — so the account is distinguishable at a glance; the model ids, thinking options, and context windows stay native.

Paseo's daemon normally reloads an agent's transcript from the profile *it* resolves (`CLAUDE_CONFIG_DIR` or `~/.claude`), ignoring the environment of the provider it spawns. The compatibility patch therefore also makes transcript replay, the session-import list, and settings-discovered models honor a provider entry's own `CLAUDE_CONFIG_DIR`, and relabels a provider's model options from its own `CLAUDE_CODEX_MODEL_LABEL_PREFIX` — settings-discovered models from the peppy profile get the prefix too. Claude Peppy conversations survive daemon restarts and app reloads, and nothing is written into the primary profile. Providers that do not pin `CLAUDE_CONFIG_DIR` — including the regular Claude provider and Claude Codex — keep the upstream resolution and unprefixed labels. `--skip-peppy` skips the launcher and provider while preserving existing ones.
...
- Provider-scoped profile resolution: a provider entry's `CLAUDE_CONFIG_DIR` steers history replay, importable sessions, and settings-discovered models, launch-time overrides win, blank values fall back, and unmarked providers keep the upstream resolution; a provider entry's `CLAUDE_CODEX_MODEL_LABEL_PREFIX` relabels its model options (ids unchanged, already-prefixed labels kept) and unmarked or blank values keep native labels.
