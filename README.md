...
The `claude-codex` **Paseo plugin** registers a `before("agent.create")` hook. When an agent is created for the Claude Codex provider with **Auto mode** explicitly requested — what the app sends when its picker shows Paseo's default for Claude providers, and what an older agent profile may still name — the hook rewrites the request to **Always Ask**, matching what Claude actually runs (see "Permission modes" above). Permission prompts appear in the app as usual, and **Accept File Edits** and **Bypass** remain available. A request that names no mode at all (for example `paseo run` without `--mode`) is left alone, so a child agent keeps inheriting its parent's mode; Paseo then applies its own default after the hook, Claude starts in `default` anyway because of the launcher's setting, and the agent's mode display follows Claude's report. The regular Claude and Claude Peppy providers are untouched. Auto mode still appears in the mode carousel, because a plugin cannot edit a built-in provider's catalog; selecting it later makes Claude run in `default` and report that. To keep auto mode for this provider, add `"CLAUDE_CODEX_AUTO_MODE": "1"` to the provider's `env` in `config.json`, run `paseo-codex plugin disable claude-codex`, and reload Paseo.
...
The launcher removes Claude's provisional input estimates from the stream so Paseo's context meter uses measured counts, and declares the **1,050,000-token** window. The context circle updates when a turn completes, from that turn's final measured usage. Because CLIProxyAPI reports Codex usage only at the end of a response, Paseo's Claude provider cannot show usage **between tool calls during a running turn**. Likewise, a skill declared with `context: fork` runs in a subagent that Claude Code does not announce through its task protocol, so Paseo's Claude provider lists it as running until the daemon restarts and shows the agents it launches flat. Both are behaviors of Paseo's built-in Claude provider that no plugin hook can reach; they belong upstream in Paseo.

### Earlier versions

Versions of this installer until mid-September 2026 edited the selected Paseo package's Claude provider module (`agent.js`) by exact string replacement to add live usage, forked-skill tracking, a mode catalog without auto mode, and profile-scoped history for the second account. That patch is gone: it was tied to the built output of one Paseo release and stopped applying on upgrades. The installer neither applies nor removes it any more. If your Paseo package still carries it, the installer says so; reinstall Paseo at the same version (`npm install -g @getpaseo/cli@$(paseo --version)`), or restore the oldest `agent.js.claude-codex-backup-*` beside the module, then restart the daemon. A Paseo upgrade replaces the module too.
...
- **Paseo lists a forked-skill subagent as working long after the agent went idle:** this is Paseo's built-in Claude provider behavior for `context: fork` skills (see "Context usage and forked skills").
...
Without integration environment variables, the real-binary tests are skipped. Node.js is used to run the Paseo plugin's hook offline; that test skips when Node is unavailable. Offline coverage includes:
...
- Paseo's own module is never modified whatever it contains, and one still carrying an earlier installer's patch is only reported.
...
Current validation on Linux (aarch64) with Python 3.12, Node 24, Claude Code 2.1.273, CLIProxyAPI 7.2.155, and a private copy of the locally installed Paseo 0.8.0: the full offline suite passes, and the gated Paseo daemon tests pass against that private copy (session completion, Ultra Code selections, turn-end context usage, the plugin loading in a real daemon and rewriting an explicit auto-mode request, and the token-selected second account replaying after a daemon restart). Earlier validation of the Plane connection and of the second-account launcher on macOS with Python 3.14 and 3.9 still applies to those unchanged parts. The proxy-only integration tests exercise the unchanged proxy and terminal launcher.
